...
if (idx === -1) throw new Error('June 2026 ga block not found')
const endMarker = '      },\n      eshop: {'
const endIdx = content.indexOf(endMarker, idx)
if (endIdx === -1) throw new Error('End of ga block not found')

const updated = content.slice(0, idx) + `      ga: ${gaJs},\n` + content.slice(endIdx)
fs.writeFileSync(DATA_FILE, updated)
console.log('Updated sanaplant.js June 2026 GA block')
