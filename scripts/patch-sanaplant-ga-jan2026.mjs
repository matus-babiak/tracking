...
if (idx === -1) throw new Error('January 2026 ga block not found')

const updated = content.slice(0, idx) + `      ga: ${gaJs},\n      email: {` + content.slice(idx + marker.length)
fs.writeFileSync(DATA_FILE, updated)
console.log(`Updated sanaplant.js January 2026 GA — ${sessions} sessions, ${totalUsers} users, ${totalRevenue?.toFixed(2)} € revenue`)
