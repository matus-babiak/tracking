...
const FILES = {
  traffic: 'Traffic_acquisition_Session_primary_channel_group_(Default_channel_group) (3).csv',
  user: 'User_acquisition_First_user_primary_channel_group_(Default_channel_group) (3).csv',
  landing: 'Landing_page_Landing_page (3).csv',
  ecommerce: 'E-commerce_purchases_Item_name (3).csv',
}
...
if (idx === -1) throw new Error('Hagardhal April 2026 ga block not found')

const updated = content.slice(0, idx) + `      ga: ${gaJs},\n      eshop: {` + content.slice(idx + marker.length)
fs.writeFileSync(DATA_FILE, updated)
console.log(`Updated hagardhal.js April 2026 GA — ${sessions} sessions, ${totalUsers} users, ${totalRevenue?.toFixed(2)} € revenue`)
